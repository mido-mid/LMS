                    ...
                    <div class="card-header bg-white border-0">
                        <div class="row align-items-center">
                            ...
                            <div class="col-4 text-right">
                                <a href="{{ route('videos.index') }}" class="btn btn-sm btn-primary">{{ __('list of videos') }}</a>
                            </div>
                        </div>
                    </div>
                    <div class="card-body">
                        <form method="post" action="{{ route('videos.update', $video) }}" autocomplete="off">
                            ...
                            <div class="pl-lg-4">
                                <div class="form-group{{ $errors->has('title') ? ' has-danger' : '' }}">
                                    ...
                                    <input type="text" name="title" id="input-title" class="form-control form-control-alternative{{ $errors->has('title') ? ' is-invalid' : '' }}" placeholder="{{ __('title') }}" value="{{ old('title', $video->title) }}" required autofocus>
                                ...
                                <div class="form-group{{ $errors->has('link') ? ' has-danger' : '' }}">
                                    ...
                                    <input type="link" name="link" id="input-link" class="form-control form-control-alternative{{ $errors->has('link') ? ' is-invalid' : '' }}" placeholder="{{ __('link') }}" value="{{ old('title', $video->link) }}" required>
                                ...
                                <div class="form-group{{ $errors->has('course_id') ? ' has-danger' : '' }}">
                                    ...
                                    <select name="course_id" required class="form-control">
                                        ...
                                        <option <?php if($video->course->id == $course->id) echo 'selected'; ?> value="{{ $course->id }}">{{ \Str::limit($course->title, 10) }}</option>
                                        @endforeach
                                    </select>
